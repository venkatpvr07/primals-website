...
function OptionSet({
  ...
  if (index === 1) {
    initialSelectedOptions = [
      optionSets[0].options === "Good" ? "Bad" : "Good",
    ];
  }
  ...
  useEffect(() => {
    ...
    if (
      ...
      data.optionSets &&
      data.optionSets[index] &&
      data.optionSets[index].options
    ) {
          ...
          <>
            <div id="flex-col">
              ...
              <Tree
                ...
                additionalOption={additionalOptions[0]}
                onSelect={handleTreeOptionChange}
              />
            </div>
...
function App() {
  ...
  useEffect(() => {
    ...
    if (optionSets[0].option === "Good") setDuplicateOption("Bad");
    else setDuplicateOption("Good");
    if (resetOptions) {
  ...
  useEffect(() => {
    if (optionSets.length == 2) setIsDuplicated(true);
  }, [optionSets.length]);
  ...
  const handleOptionChange = useCallback(function handleOptionChange(
    options,
    index
  ) {
    ...
    if (options[0] === "Bad") {
      ...
      setInstruction(
        "You chose Bad. Now, choose one secondary from Dangerous, Dull and Mechanistic."
      );
    } else if (options[0] === "Good") {
      setDuplicateOption("Bad");
      setInstruction(
        "You chose Good. Now, choose one secondary from Safe, Enticing and Alive."
      );
    }
    // if(options[0]==='Good')
  ...
  const handleAdditionalOptionChange = useCallback(
    function handleAdditionalOptionChange(additionalOptions, index) {
  ...
  const handleTreeOptionChange = (selectedOption, index) => {
    ...
    if (selectedOption.length > 0)
      setInstruction(
        "Great! Now save your selections by clicking on Save Annotations button."
      );
    setOptionSets((prevOptionSets) => {
      let currentOptionSets = [{ ...prevOptionSets[0] }];
      if (index === 1)
  ...
  const handleDuplicateOptions = () => {
    if (
      ...
      alert(
        "Please select one primary, one secondary, and one tertiary primals before saving annotations."
      );
      console.log(
        "Please select one primary, one secondary, and one tertiary primals before saving annotations."
      );
  ...
  const handleSaveAnnotations = async () => {
    ...
    try {
      ...
      if (response.ok) {
        setInstruction(
          "Done! You can go to the next page or add another set of annotations by clicking on Add Good / Add Bad button."
        );
  ...
  const handleNext = async () => {
    try {
      ...
      alert(
        "Error occurred while saving annotations. Check console for details."
      );
    }
  };
  ...
  const handleSubmit = async () => {
    try {
      if (!savedPages.includes(currentPage)) {
        ...
        alert(
          "Data saved. You reached the end, now you may close the browser."
        );
      }
  ...
  return (
    <div className="app-container">
      <textarea className="text-box" rows="5" value={text} readOnly />
      ...
      <div className="main-content">
        ...
        <footer className="footer">
          <button
            ...
            onClick={handleDuplicateOptions}
            disabled={isDuplicated}
          >
            Add {duplicateOption}{" "}
          </button>
          <button className="footer-button" onClick={handleSaveAnnotations}>
            Save Annotations
          </button>
          ...
          {showSubmit && (
            <button
              ...
              disabled={savedPages.length === totalPages ? false : true}
            >
              Submit
            </button>
          )}
          ...
          {pageData.map((page, index) => (
            <li
              ...
              style={{
                ...
                padding: "4px",
                cursor: "pointer",
                backgroundColor: currentPage === index + 1 ? "#bcbcbc" : "#eee",
